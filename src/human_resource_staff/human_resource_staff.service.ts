...
if(search){
  ...
  query+= `WHERE (staff.id LIKE ? OR staff.name LIKE ? OR roles.name LIKE ? OR staff.employee_id LIKE ? OR staff.contact_no LIKE ? OR staff.email LIKE ?) AND staff.is_active = 1 `
  values.push("%"+search+"%")
  values.push("%"+search+"%")
  values.push("%"+search+"%")
  values.push("%"+search+"%")
...
async findByDisabledStaffIdNameRole(search: string): Promise<HumanResourceStaff[]> {
  ...
  if(search){
    ...
    query+= `WHERE (staff.id LIKE ? OR staff.name LIKE ? OR roles.name LIKE ? OR staff.employee_id LIKE ? OR staff.contact_no LIKE ? OR staff.email LIKE ?) AND staff.is_active = 0 `
    values.push("%"+search+"%")
    values.push("%"+search+"%")
    values.push("%"+search+"%")
    values.push("%"+search+"%")
