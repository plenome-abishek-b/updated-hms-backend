...
export class SetupAppointmentDoctorShiftService {
  ...
  async create(
    ...
    try {
      ...
      const dynamicConnectionOptions: MysqlConnectionOptions =
        dynamicDbConfig as MysqlConnectionOptions;
      dynamicConnection = await createConnection(dynamicConnectionOptions);

      const [staffEmail] = await this.connection.query(
        `select email from staff where id = ?`,
        [doctor_shiftEntity.staff_id],
      );
      const [staff_email] = await dynamicConnection.query(
        `select id from staff where email = ?`,
        [staffEmail.email],
      );
      const [doctor_global_shift] = await dynamicConnection.query(
      ...
      if (check) {
        const [GetAdmin] = await dynamicConnection.query(
          `select id from doctor_global_shift where Hospital_id = ? and hospital_doctor_global_shift_id = ?`,
          [doctor_shiftEntity.Hospital_id, check.id],
        );
        console.log(GetAdmin, 'getAdmin');
        const delAdmin = await dynamicConnection.query(
          `delete from doctor_global_shift where id = ?`,
          [GetAdmin.id],
        );
        const del = await this.connection.query(
          `delete from doctor_global_shift where id = ?`,
          [check.id],
        );
        return [
        ...
        const AdminCategory = await dynamicConnection.query(
          `INSERT INTO doctor_global_shift (staff_id,global_shift_id,Hospital_id,hospital_doctor_global_shift_id) VALUES (?,?,?,?)`,
          [
            staff_email.id,
        ...
        return [
          {
            'data ': {
              ...
              inserted_data: await this.connection.query(
                'SELECT * FROM doctor_global_shift WHERE id = ?',
                [result.insertId],
              ),
            },
          },
