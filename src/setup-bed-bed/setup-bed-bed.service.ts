...
const [bed_group] = await dynamicConnection.query(`select id from bed_group where Hospital_id =? and hospital_bed_group_id = ?`,[
  ...
  async findAll(): Promise<SetupBedBed[]> {
    const bed = await this.connection.query("SELECT bed.id,bed.name, bed.is_active as used,bed.bed_type_id, bed_type.name as Bed_Type,bed.bed_group_id, CONCAT(bed_group.name, '-', floor.name) AS bed_group FROM bed JOIN bed_type ON bed.bed_type_id = bed_type.id JOIN bed_group ON bed.bed_group_id = bed_group.id JOIN floor ON bed_group.floor = floor.id");
    console.log(bed,"bed");
